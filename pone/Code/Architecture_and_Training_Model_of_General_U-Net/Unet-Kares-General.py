...
output_name = os.listdir('H:/Data/TrainingDataSet/General-1-channel-Labels')
n = len(input_name)
batch_size = 8
input_size_1 = 256
input_size_2 = 256
...
def batch_data(input_name, n, batch_size = 8, input_size_1 = 256, input_size_2 = 256):
    ...
    img2 = io.imread('H:/Data/TrainingDataSet/General-1-channel-Labels/'+output_name[rand_num]).astype("float")
...
outpt = Conv2D(filters=3, kernel_size=(1,1), strides=(1,1), padding='same', activation='softmax')(conv9)

# log_filepath = 'H:/my_image_test/third_breast/experiment results/unet5/6/log'
...
for i in range(itr):
    ...
    history = model.fit(train_X, train_Y, epochs=1, verbose=2)                          #应用Tensorboard


#with open("H:/my_image_test/third_breast/experiment results/unet5/3/trainHistoryDict.txt","wb") as file_pi:
  # pickle.dump(history.history, file_pi)        #保存训练记录

#with open("H:/my_image_test/third_breast/experiment results/unet5/3/trainHistoryDict.txt","rb") as file_pi:
  # history = pickle.load(file_pi)     #读取训练记录

save_model(model, 'H:/Data/Code\Architecture_and_Training_Model_of_General_U-Net/unet1.h5')



#TEST

model = load_model('H:/Data/Code\Architecture_and_Training_Model_of_General_U-Net/unet1.h5')

test_input_name = os.listdir('H:/Data/TestingDataSet/Test-General-1-BreastTumourImages')
test_output_name = os.listdir('H:/Data/TestingDataSet/Test-General-1-channel-Labels')
n_test = len(test_input_name)
def batch_data_test(input_name, n, batch_size = 4, input_size_1 = 256, input_size_2 = 256):
    rand_num = random.randint(0, n-1)
    img1 = io.imread('H:/Data/TestingDataSet/Test-General-1-BreastTumourImages/'+test_input_name[rand_num]).astype("float")
    img2 = io.imread('H:/Data/TestingDataSet/Test-General-1-channel-Labels/'+test_output_name[rand_num]).astype("float")
    img1 = resize(img1, [input_size_1, input_size_2, 3])
    img2 = resize(img2, [input_size_1, input_size_2, 1])
    img1 = np.reshape(img1, (1, input_size_1, input_size_2, 3))
    img2 = np.reshape(img2, (1, input_size_1, input_size_2, 1))
    ...
    for batch_iter in range(1, batch_size):
        ...
        img2 = io.imread('H:/Data/TestingDataSet/Test-General-1-channel-Labels/'+test_output_name[rand_num]).astype("float")
        img1 = resize(img1, [input_size_1, input_size_2, 3])
        img2 = resize(img2, [input_size_2, input_size_2, 1])
        img1 = np.reshape(img1, (1, input_size_1, input_size_2, 3))
        img2 = np.reshape(img2, (1, input_size_2, input_size_2, 1))
        img1 /= 255
        batch_input = np.concatenate((batch_input,img1), axis = 0)
        batch_output = np.concatenate((batch_output, img2), axis =0)
    return batch_input, batch_output
...
plt.axis('off')
plt.imsave('H:/Data/Segmentation_Results/General_U-Net/10/28.jpg',test_X[ii, :, :, :])
plt.figure()
plt.imshow(test_Y[ii, :, :, :])
plt.axis('off')
plt.imsave('H:/Data/Segmentation_Results/General_U-Net/10/29.jpg',test_Y[ii, :, :, :])
plt.figure()
plt.imshow(pred_Y[ii, :, :, :])
plt.axis('off')
plt.imsave('H:/Data/Segmentation_Results/General_U-Net/10/30.jpg',pred_Y[ii, :, :, :])
plt.show()
sys.exit(0)


# input_single_x = os.listdir('H:/my_image_test/third_breast/single_input')
# n_single = len(input_single_x)

# rand_num_single = random.randint(0, n_single-1)

# input_single_x = io.imread('H:/my_image_test/third_breast/single_input/'+input_single_x[rand_num_single]).astype("float")
